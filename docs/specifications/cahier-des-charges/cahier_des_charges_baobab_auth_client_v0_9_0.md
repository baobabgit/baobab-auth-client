# Cahier des charges — `baobab-auth-client` — version `v0.9.0`
...
**Titre :** Introspection distante, révocation et release candidate

Préparer la stabilisation `1.0.0` avec les derniers mécanismes optionnels : introspection distante de token, vérification de révocation, durcissement des erreurs, observabilité et tests end-to-end.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<1.0.0` | Préparation compatibilité finale. |
| `httpx` | `>=0.27,<1.0` | Introspection distante optionnelle. |
| `fastapi` | `>=0.110,<1.0` | Tests API consommatrice. |
| `PyJWT[crypto]` | `>=2.8,<3.0` | Validation JWT locale. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-api` | `0.9.0` | Validation endpoints stables et éventuel endpoint d’introspection/révocation si disponible. |
| `baobab-auth-security` | `0.5.0` | Validation révocation par `jti`, `sid`, `sub`, `kid` et rotation de clés. |
| `baobab-auth-service` | `0.3.0` | Validation end-to-end Docker. |
| `baobab-auth-admin` | `0.1.0` | Validation indirecte des scénarios rotation/révocation exposés côté API. |
...
## 5. Périmètre fonctionnel inclus dans `v0.9.0`

- Ajouter introspection distante optionnelle si endpoint disponible.
- Ajouter vérification de révocation optionnelle.
- Ajouter hooks de logging/metrics non sensibles.
- Durcir messages d’erreur.
- Créer tests end-to-end contre service Docker.
- Rédiger matrice finale de compatibilité.
- Déprécier toute API instable avant `1.0.0`.

---

## 5.1 Hors périmètre de `v0.9.0`

- Forcer introspection distante sur tous les tokens.
- Stocker localement une denylist persistante.
- Implémenter une API d’administration.
...
### 9.1 `client/introspection.py`

- Créer client optionnel pour `/auth/introspect` si route disponible.
- Prévoir comportement désactivé par défaut.
- Gérer indisponibilité sans affaiblir la validation locale.

### 9.2 `tokens/revocation.py`

- Vérifier révocation par appel API optionnel ou cache court.
- Ne jamais considérer un échec réseau comme token valide en mode strict.

### 9.3 `observability.py`

- Centraliser logs et compteurs optionnels.
- Masquer tokens et secrets.
...
- `test_remote_introspection_success`
- `test_remote_introspection_disabled_by_default`
- `test_revoked_token_rejected_in_strict_mode`
- `test_revocation_network_failure_strict_rejects`
- `test_revocation_network_failure_tolerant_policy_documented`
- `test_key_rotation_refreshes_jwks`
- `test_e2e_docker_login_then_call_protected_route`
- `test_public_api_deprecations_documented`
...
### BL-C-041 — Ajouter introspection distante optionnelle.
...
### BL-C-042 — Ajouter vérification révocation optionnelle.
...
### BL-C-043 — Ajouter observabilité non sensible.
...
### BL-C-044 — Créer tests end-to-end Docker.
...
### BL-C-045 — Préparer release candidate et dépréciations.
...
## 14. Critères d’acceptation de la version `v0.9.0`
...
Après validation de `v0.9.0`, toutes les briques majeures peuvent préparer leurs versions candidates. Les contrats client/API/security/service doivent être considérés quasi gelés avant `1.0.0`.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.9.0
...
La version `v0.9.0` est terminée lorsque :
...
7. un tag Git `v0.9.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.9.0`.
