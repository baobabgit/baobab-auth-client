# Cahier des charges — `baobab-auth-client` — version `v0.2.0`
...
**Titre :** Client HTTP asynchrone vers baobab-auth-api

Ajouter le client HTTP asynchrone `AuthClient` pour consommer les endpoints principaux de `baobab-auth-api`. Cette version valide le contrat HTTP minimal : login, refresh, logout, me, jwks et health.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<0.5.0` | Continuité des types et concepts utilisés par les modèles client. |
| `httpx` | `>=0.27,<1.0` | Dépendance obligatoire pour l’extra `http` et le client async. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-api` | `0.1.0` | Validation du contrat HTTP minimal : `/auth/login`, `/auth/logout`, `/auth/refresh`, `/auth/me`, `/auth/jwks`, `/health`. |
| `baobab-auth-core` | `0.4.0` | Validation de la cohérence des DTO exposés ou mappés côté client. |
| `baobab-auth-security` | `0.1.0` | Validation indirecte du format `TokenPair` retourné par l’API, même si les tokens sont encore opaques côté client. |
...
## 5. Périmètre fonctionnel inclus dans `v0.2.0`

- Créer `client/async_client.py`.
- Créer les schémas de requête/réponse dans `client/schemas.py`.
- Implémenter `login`, `refresh`, `logout`, `me`, `jwks`, `health`.
- Gérer les erreurs HTTP.
- Gérer les timeouts.
- Permettre l’injection d’un `httpx.AsyncClient`.
- Supporter le context manager async.
- Masquer les tokens et mots de passe dans logs/exceptions.

---

## 5.1 Hors périmètre de `v0.2.0`
...
### 9.1 `client/async_client.py`

- Créer `AuthClient` avec `base_url`, `settings`, `http_client`, `timeout`.
- Implémenter `__aenter__`, `__aexit__`, `aclose`.
- Centraliser la construction des URLs.
- Centraliser la conversion des erreurs HTTP.

### 9.2 `client/schemas.py`

- Définir `LoginRequest`, `RefreshRequest`, `LogoutRequest` si nécessaire.
- Définir les parsers de `TokenPair`, `AuthenticatedUser`, `AuthHealth`.
- Valider les réponses JSON incomplètes.

### 9.3 `client/errors.py`

- Mapper 400/401/403/404/409/422/500 vers exceptions client.
...
- `test_login_401_raises_auth_api_error_without_password`
- `test_refresh_success`
- `test_logout_success`
- `test_me_success_returns_authenticated_user`
- `test_jwks_success_returns_mapping`
- `test_health_success`
- `test_timeout_raises_auth_api_unavailable`
- `test_invalid_json_response_raises_auth_api_error`
- `test_async_client_can_be_injected`
...
### BL-C-006 — Ajouter dépendance optionnelle `httpx` et extra `http`.
...
### BL-C-007 — Implémenter `AuthClient` async et context manager.
...
### BL-C-008 — Implémenter schémas et parsers HTTP.
...
### BL-C-009 — Implémenter mapping des erreurs API.
...
### BL-C-010 — Ajouter tests avec mock HTTP.
...
## 14. Critères d’acceptation de la version `v0.2.0`
...
Après validation de `v0.2.0`, `baobab-auth-api v0.1.0` peut être considéré compatible avec un client Python externe. Les évolutions de l’API devront préserver ou versionner ce contrat.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.2.0
...
La version `v0.2.0` est terminée lorsque :
...
7. un tag Git `v0.2.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.2.0`.
