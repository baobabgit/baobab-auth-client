# Cahier des charges — `baobab-auth-client` — version `v0.8.0`
...
**Titre :** Middleware Starlette/FastAPI et intégration applicative avancée

Ajouter un middleware optionnel permettant d’attacher automatiquement l’utilisateur authentifié au `request.state`, tout en conservant les dépendances FastAPI existantes.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<0.9.0` | Claims et utilisateur authentifié. |
| `starlette` | `>=0.37,<1.0` | Middleware ASGI. |
| `fastapi` | `>=0.110,<1.0` | Compatibilité FastAPI. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-api` | `0.7.0` | Validation dans une API consommatrice avec middleware. |
| `baobab-auth-security` | `0.4.0` | Validation JWKS et rotation simulée avec middleware. |
| `API consommatrice pilote` | `0.2.0` | Validation middleware + dépendances sur routes métier. |
...
## 5. Périmètre fonctionnel inclus dans `v0.8.0`

- Créer `starlette/middleware.py`.
- Attacher `request.state.auth_user`.
- Permettre mode obligatoire ou optionnel.
- Permettre exclusion de chemins publics.
- Préserver les dépendances FastAPI.
- Documenter ordre des middlewares.
- Tester performances de base et absence d’appel API par requête si JWKS en cache.

---

## 5.1 Hors périmètre de `v0.8.0`

- Session cookies.
- CSRF.
- Authentification WebSocket complète.
...
### 9.1 `starlette/middleware.py`

- Implémenter `AuthenticationMiddleware` spécifique Baobab ou nom non conflictuel.
- Gérer chemins exclus `/health`, `/docs`, `/openapi.json` par configuration.
- Ne pas transformer toutes les erreurs en 500.

### 9.2 `fastapi/dependencies.py`

- Permettre aux dépendances de réutiliser `request.state.auth_user` si présent.
- Éviter double validation token dans la même requête.
...
- `test_middleware_attaches_user_to_request_state`
- `test_middleware_optional_mode_allows_anonymous`
- `test_middleware_required_mode_rejects_missing_token`
- `test_excluded_path_does_not_validate_token`
- `test_dependency_reuses_request_state_user`
- `test_invalid_token_returns_401`
- `test_middleware_does_not_log_token`
...
### BL-C-036 — Créer extra `starlette` si nécessaire.
...
### BL-C-037 — Implémenter middleware ASGI.
...
### BL-C-038 — Adapter dépendances pour réutiliser `request.state`.
...
### BL-C-039 — Ajouter tests middleware.
...
### BL-C-040 — Documenter intégration avancée.
...
## 14. Critères d’acceptation de la version `v0.8.0`
...
Après validation de `v0.8.0`, les APIs consommatrices peuvent choisir entre dépendances route par route et middleware global. `baobab-auth-api v0.7.0` est validée pour usage applicatif avancé.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.8.0
...
La version `v0.8.0` est terminée lorsque :
...
7. un tag Git `v0.8.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.8.0`.
