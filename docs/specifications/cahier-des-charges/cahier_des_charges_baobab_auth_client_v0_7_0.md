# Cahier des charges — `baobab-auth-client` — version `v0.7.0`
...
**Titre :** Multi-audience, multi-issuer et configuration avancée

Permettre l’usage de `baobab-auth-client` dans des environnements multi-applications : plusieurs audiences, plusieurs issuers, plusieurs URLs JWKS et règles de sélection par route ou par application.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<0.8.0` | Rôles/permissions et claims compatibles. |
| `httpx` | `>=0.27,<1.0` | JWKS multiples. |
| `PyJWT[crypto]` | `>=2.8,<3.0` | Validation tokens. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-api` | `0.6.0` | Validation des claims `aud` multiples ou audiences configurées. |
| `baobab-auth-security` | `0.4.0` | Validation émission JWT compatible multi-audience et rotation de clés. |
| `baobab-auth-service` | `0.2.0` | Validation environnement multi-service via Docker. |
...
## 5. Périmètre fonctionnel inclus dans `v0.7.0`

- Supporter plusieurs issuers configurés.
- Supporter plusieurs audiences autorisées.
- Supporter plusieurs JWKS URLs.
- Créer un `TokenValidatorRegistry` ou équivalent.
- Permettre sélection par issuer/audience.
- Isoler les caches JWKS par issuer.
- Tester les erreurs de confusion issuer/audience.
- Documenter configuration multi-service.

---

## 5.1 Hors périmètre de `v0.7.0`

- Fédération OAuth externe.
- OIDC discovery complet.
- Multi-tenant avec isolation base de données.
...
- Ajouter configuration multi-issuer si nécessaire.
- Conserver compatibilité avec configuration simple.

### 9.2 `tokens/registry.py`

- Créer un registre de validateurs par issuer.
- Empêcher fallback dangereux vers un issuer par défaut si le token en annonce un autre.

### 9.3 `jwks/cache.py`

- Isoler les caches par issuer/JWKS URL.
- Prévenir la réutilisation d’une clé d’un issuer pour un autre.
...
- `test_single_issuer_config_still_works`
- `test_multiple_audiences_accepts_configured_audience`
- `test_wrong_audience_rejected`
- `test_wrong_issuer_rejected`
- `test_jwks_cache_isolated_by_issuer`
- `test_no_key_confusion_between_issuers`
- `test_multi_issuer_fastapi_dependency`
...
### BL-C-031 — Étendre configuration multi-audience.
...
### BL-C-032 — Créer registre de validateurs.
...
### BL-C-034 — Ajouter tests anti-confusion issuer/audience.
...
### BL-C-035 — Documenter multi-service et exemples.
...
## 14. Critères d’acceptation de la version `v0.7.0`
...
Après validation de `v0.7.0`, `baobab-auth-security v0.4.0` et `baobab-auth-api v0.6.0` peuvent poursuivre les scénarios multi-service sans casser les APIs consommatrices.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.7.0
...
La version `v0.7.0` est terminée lorsque :
...
7. un tag Git `v0.7.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.7.0`.
