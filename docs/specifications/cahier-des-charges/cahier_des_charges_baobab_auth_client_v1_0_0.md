# Cahier des charges — `baobab-auth-client` — version `v1.0.0`
...
**Titre :** Stabilisation API publique et validation inter-briques finale

Publier la première version stable de `baobab-auth-client`. Cette version doit figer l’API publique, garantir la compatibilité avec les versions stables des briques `baobab-auth`, documenter les contrats, fournir une couverture de tests élevée et permettre l’intégration fiable dans des APIs métier réelles.
...
|---|---:|---|
| `baobab-auth-core` | `>=1.0.0,<2.0.0` | Contrat métier stable. |
| `httpx` | `>=0.27,<1.0` | Client HTTP stable. |
| `PyJWT[crypto]` | `>=2.8,<3.0` | Validation JWT stable. |
| `fastapi` | `>=0.110,<1.0` | Extra FastAPI stable. |
| `starlette` | `>=0.37,<1.0` | Middleware optionnel stable. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-core` | `1.0.0` | Validation finale des types, rôles, permissions, AuthSubject et DTO compatibles. |
| `baobab-auth-api` | `1.0.0` | Validation finale des endpoints login/refresh/logout/me/jwks/health/introspection si activée. |
| `baobab-auth-security` | `1.0.0` | Validation finale JWT/JWKS/rotation/révocation et claims. |
| `baobab-auth-service` | `1.0.0` | Validation end-to-end dans Docker. |
| `baobab-auth-database` | `1.0.0` | Validation indirecte via API/service : sessions, rôles, permissions, audit et clés persistées. |
| `baobab-auth-admin` | `1.0.0` | Validation indirecte des scénarios admin consommés par API : révocation, rotation, rôles. |
| `API métier pilote` | `1.0.0 ou pré-1.0` | Validation réelle d’une API consommatrice avec routes protégées. |
...
## 5. Périmètre fonctionnel inclus dans `v1.0.0`

- Figer l’API publique.
- Documenter la politique de compatibilité semver.
- Vérifier tous les exports publics.
- Vérifier la matrice de compatibilité inter-briques.
- Atteindre couverture minimale `>=95 %` recommandée.
- Garantir `ruff`, `mypy`, `pytest` sans échec.
- Rédiger guides complets d’installation et d’intégration.
- Publier changelog stable.
- Préparer tag Git `v1.0.0` et release PyPI.

---

## 5.1 Hors périmètre de `v1.0.0`

- Changer les noms publics sans migration.
- Ajouter de grands nouveaux modules non testés.
- Casser les contrats FastAPI existants.
...
- Auditer `__all__` et exports.
- Stabiliser noms `AuthClient`, `SyncAuthClient`, `TokenValidator`, `JwksCache`, `AuthenticatedUser`.
- Documenter toute API expérimentale comme telle ou la retirer.

### 9.2 `docs/`

- Finaliser `usage.md`, `configuration.md`, `fastapi.md`, `jwt-validation.md`, `jwks.md`, `testing.md`, `middleware.md`, `integration-matrix.md`.
- Ajouter exemples complets minimal API.

### 9.3 `tests/contract/`

- Créer matrice de tests contre versions stables des briques auth.
- Valider tokens réels et JWKS réels.
...
- `test_public_api_exports_are_stable`
- `test_semver_compatibility_matrix`
- `test_e2e_auth_api_login_and_protected_route`
- `test_e2e_jwks_rotation_refresh`
- `test_e2e_revoked_session_rejected`
- `test_fastapi_require_user_stable`
- `test_middleware_stable`
- `test_testing_helpers_stable`
- `test_no_secret_in_logs_or_exceptions`
- `test_docs_examples_execute`
...
### BL-C-046 — Auditer et figer API publique.
...
### BL-C-047 — Finaliser matrice de compatibilité inter-briques.
...
### BL-C-048 — Renforcer couverture et tests contractuels.
...
### BL-C-049 — Finaliser documentation stable.
...
### BL-C-050 — Préparer release `v1.0.0` et publication.
...
## 14. Critères d’acceptation de la version `v1.0.0`
...
Après validation de `v1.0.0`, `baobab-auth-client` peut être utilisé comme dépendance stable par les APIs métier. Les briques `core`, `api`, `security`, `database`, `admin` et `service` disposent d’un retour d’intégration final côté client.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v1.0.0
...
La version `v1.0.0` est terminée lorsque :
...
7. un tag Git `v1.0.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v1.0.0`.
