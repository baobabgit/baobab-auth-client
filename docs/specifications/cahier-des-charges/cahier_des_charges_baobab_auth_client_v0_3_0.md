# Cahier des charges — `baobab-auth-client` — version `v0.3.0`
...
**Titre :** JWKS, cache de clés et validation JWT locale

Permettre à une API consommatrice de valider localement un access token JWT sans rappeler l’API auth à chaque requête. Cette version introduit `JwksFetcher`, `JwksCache`, `TokenValidator` et le parsing sécurisé des claims.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<0.5.0` | Validation des concepts `AuthSubject`, `TokenClaims`, rôles et permissions. |
| `httpx` | `>=0.27,<1.0` | Récupération JWKS. |
| `PyJWT[crypto]` | `>=2.8,<3.0` | Validation JWT avec clés asymétriques. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-security` | `0.1.0` | Validation de la compatibilité des JWT émis : `sub`, `sid`, `jti`, `roles`, `permissions`, `iat`, `exp`, `iss`, `aud`, `kid`. |
| `baobab-auth-api` | `0.2.0` | Validation de l’endpoint `/auth/jwks` et du format JWKS exposé. |
| `baobab-auth-core` | `0.4.0` | Validation du mapping claims → utilisateur authentifié côté client. |
...
## 5. Périmètre fonctionnel inclus dans `v0.3.0`

- Créer `jwks/fetcher.py`.
- Créer `jwks/cache.py`.
- Créer `tokens/parser.py`.
- Créer `tokens/validator.py`.
- Valider signature, issuer, audience, expiration, nbf, iat.
- Construire `AuthenticatedUser` depuis les claims.
- Supporter refresh JWKS sur `kid` inconnu.
- Limiter les refreshs abusifs du cache.
- Tester les tokens valides et invalides.

---

## 5.1 Hors périmètre de `v0.3.0`

- Révocation distante obligatoire.
...
- Définir `JwksDocument`, `JwkKey` ou modèles internes équivalents.
- Valider que `keys` est présent et que chaque clé contient `kid`, `kty`, `alg` si requis.

### 9.2 `jwks/fetcher.py`

- Appeler `GET /auth/jwks`.
- Gérer timeout et réponse invalide.
- Ne jamais stocker de clé privée.

### 9.3 `jwks/cache.py`

- Gérer TTL.
- Rechercher une clé par `kid`.
- Forcer refresh sur `kid` inconnu une seule fois par validation.
- Prévoir `clear()` pour tests et rotation.

### 9.4 `tokens/validator.py`

- Décoder header JWT sans validation pour lire `kid` et `alg`.
- Refuser `alg=none`.
- Valider signature via JWKS.
- Mapper les exceptions PyJWT vers exceptions client.
- Construire `AuthenticatedUser`.
...
- `test_jwks_fetch_success`
- `test_jwks_fetch_timeout`
- `test_jwks_cache_hit`
- `test_jwks_cache_ttl_expired_refreshes`
- `test_unknown_kid_triggers_single_refresh`
- `test_valid_jwt_returns_authenticated_user`
- `test_expired_jwt_raises_expired_token_error`
- `test_invalid_issuer_raises`
- `test_invalid_audience_raises`
- `test_invalid_signature_raises`
- `test_alg_none_is_rejected`
- `test_service_role_with_empty_permissions_is_valid`
...
### BL-C-011 — Implémenter modèles JWKS et fetcher.
...
### BL-C-012 — Implémenter cache JWKS avec TTL et refresh contrôlé.
...
### BL-C-013 — Implémenter parser JWT sécurisé.
...
### BL-C-014 — Implémenter `TokenValidator`.
...
### BL-C-015 — Créer jeux de tests cryptographiques et JWKS mockés.
...
## 14. Critères d’acceptation de la version `v0.3.0`
...
Après validation de `v0.3.0`, `baobab-auth-security v0.1.0` et `baobab-auth-api v0.2.0` peuvent poursuivre leurs développements en conservant le contrat JWT/JWKS validé par un client réel.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.3.0
...
La version `v0.3.0` est terminée lorsque :
...
7. un tag Git `v0.3.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.3.0`.
