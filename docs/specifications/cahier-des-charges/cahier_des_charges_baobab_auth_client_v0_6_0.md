# Cahier des charges — `baobab-auth-client` — version `v0.6.0`
...
**Titre :** Client synchrone, transport configurable et robustesse réseau

Ajouter une façade synchrone et renforcer la robustesse réseau : transport injectable, retries optionnels, timeouts homogènes, hooks de monitoring et compatibilité stricte avec les environnements non-async.
...
|---|---:|---|
| `baobab-auth-core` | `>=0.4.0,<0.7.0` | Compatibilité maintenue avec DTO et value objects. |
| `httpx` | `>=0.27,<1.0` | Client async et sync. |
| `tenacity` | `optionnel >=8,<10` | Retries optionnels si retenu, sinon implémentation interne simple. |

### 4.2 Librairies `baobab-auth` à valider pour poursuivre les développements

| Librairie | Version à utiliser / valider | Rôle dans cette version |
|---|---:|---|
| `baobab-auth-api` | `0.5.0` | Validation du comportement réseau, erreurs normalisées et healthcheck. |
| `baobab-auth-service` | `0.1.0` | Validation dans un environnement Docker d’assemblage auth-api + auth-db. |
| `baobab-auth-security` | `0.3.0` | Validation stable des JWKS malgré refresh et rotation simulée. |
...
## 5. Périmètre fonctionnel inclus dans `v0.6.0`

- Créer `client/sync_client.py`.
- Factoriser la logique commune async/sync.
- Permettre transport injectable.
- Ajouter stratégie de timeout centralisée.
- Ajouter retries optionnels sur endpoints non sensibles.
- Ne jamais rejouer automatiquement `login` sans décision explicite.
- Ajouter logs structurés non sensibles.
- Tester erreurs réseau et fermeture des ressources.

---

## 5.1 Hors périmètre de `v0.6.0`
...
### 9.1 `client/sync_client.py`

- Créer `SyncAuthClient` ou `AuthClientSync`.
- Implémenter `login`, `refresh`, `logout`, `me`, `jwks`, `health` en sync.
- Supporter context manager `with`.

### 9.2 `client/base.py`

- Factoriser mapping erreurs, URLs et parsers.
- Éviter duplication excessive async/sync.

### 9.3 `client/retry.py`

- Définir les endpoints retryables.
- Ne pas retry automatiquement les opérations qui pourraient créer des effets indésirables sans idempotency key.
...
- `test_sync_login_success`
- `test_sync_context_manager_closes_client`
- `test_async_and_sync_parse_same_token_pair`
- `test_timeout_configuration_applied`
- `test_retry_health_transient_failure`
- `test_no_password_in_retry_logs`
- `test_network_error_maps_to_unavailable`
...
### BL-C-026 — Factoriser client HTTP commun.
...
### BL-C-027 — Implémenter client synchrone.
...
### BL-C-028 — Ajouter stratégie timeout/retry configurable.
...
### BL-C-029 — Ajouter tests robustesse réseau.
...
### BL-C-030 — Documenter usages async et sync.
...
## 14. Critères d’acceptation de la version `v0.6.0`
...
Après validation de `v0.6.0`, `baobab-auth-api v0.5.0` et `baobab-auth-service v0.1.0` sont validés pour consommation réseau robuste depuis des clients Python async et sync.

L’IA de développement doit produire en fin de version un résumé d’intégration au format :

```markdown
## Résultat d’intégration v0.6.0
...
La version `v0.6.0` est terminée lorsque :
...
7. un tag Git `v0.6.0` peut être créé ;
...
Traiter ce cahier des charges comme contractuel pour `baobab-auth-client v0.6.0`.
